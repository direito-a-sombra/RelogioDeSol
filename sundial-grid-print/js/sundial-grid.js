...
const elLat = document.getElementById("sundial-lat");
const elLon = document.getElementById("sundial-lon");
const elTz = document.getElementById("sundial-tz");
...
function renderGrid() {
  ...
  for (let m = mMin; m <= mMax; m+=mStep) {
    ...
    for (let hh = hhMin; hh <= hhMax; hh += hhStep) {
      ...
      const out = computeAlphaBeta(y, m, d, hh, mm, poleH, lat, lon, tz);
