...
UI.lat.value = LAT_DEG;
UI.lon.value = LON_DEG;
UI.tz.value = TZ_OFFSET_HOURS;
UI.date.value = todayStr(UI.tz.value);
UI.time.value = nowTime(UI.tz.value);

function readLocalParts() {
  const tz = parseFloat(UI.tz.value);
  const d = UI.date.value ? UI.date.value : todayStr(tz);
  const [Y, M, D] = d.split("-").map(n => parseInt(n));
  const [hh, mm] = (UI.time.value || "12:00").split(":").map(n => parseInt(n));
  const latDeg = parseFloat(UI.lat.value);
  const lonDeg = parseFloat(UI.lon.value);
  return { Y, M, D, hh, mm, latDeg, lonDeg, tz };
}

//////// 3D Scene
const canvas = $("sundial-3d-renderer");
...
function update() {
  ...
  const { Y, M, D, hh, mm, latDeg, lonDeg, tz } = readLocalParts();
  const sp = solarAzElNoaa(Y, M, D, hh, mm, 0, latDeg, lonDeg, tz);
...
//////// Events
const UIList = [UI.date, UI.time, UI.pole, UI.lat, UI.lon, UI.tz];
window.addEventListener("resize", resize);
UIList.forEach(el => el.addEventListener("keydown", (evt) => evt.stopPropagation()));
["input", "change"].forEach(ev => { UIList.forEach(el => el.addEventListener(ev, update)) });
